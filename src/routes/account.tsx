    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      {confirmCancelId && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setConfirmCancelId(null)}>
          <div className="bg-background border border-hairline w-full max-w-sm p-8 text-center" onClick={(e) => e.stopPropagation()}>
            <h2 className="font-serif text-2xl mb-3">Cancel Order?</h2>
            <p className="text-sm text-ink-soft mb-8">This action cannot be undone. The order will be marked as cancelled.</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmCancelId(null)} className="btn-ghost flex-1 justify-center">Keep Order</button>
              <button onClick={() => cancelOrder(confirmCancelId)} className="flex-1 h-12 border border-destructive text-destructive text-sm tracking-wide hover:bg-destructive hover:text-destructive-foreground transition">Yes, Cancel</button>
            </div>
          </div>
        </div>
      )}
