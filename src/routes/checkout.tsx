    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-24 md:pt-32 max-w-[1200px] mx-auto px-6 md:px-10 pb-24">
        ...
        <div className="flex items-center gap-0 mb-10 max-w-xs">
          {(["Bag", "Details", "Confirm"] as const).map((step, i) => (
            <div key={step} className="flex items-center gap-0 flex-1">
              <div className={`flex items-center gap-2 ${i <= 1 ? "text-ink" : "text-ink-soft"}`}>
                <span className={`w-6 h-6 rounded-full text-[10px] flex items-center justify-center font-medium ${i <= 1 ? "bg-primary text-primary-foreground" : "border border-hairline"}`}>{i + 1}</span>
                <span className="text-[11px] tracking-[0.15em] uppercase hidden sm:block">{step}</span>
              </div>
              {i < 2 && <span className="flex-1 h-px bg-hairline mx-2" />}
            </div>
          ))}
        </div>
