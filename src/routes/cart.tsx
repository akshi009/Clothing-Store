        ...
        {items.length === 0 ? (
          ...
          <div className="grid lg:grid-cols-[1fr_380px] gap-10">
            <ul className="divide-y divide-hairline border-y border-hairline">
              {items.map((i) => (
                <li key={`${i.id}-${i.size ?? ""}`} className="py-6 flex gap-5">
                  {i.productId ? (
                    <Link to="/product/$id" params={{ id: i.productId }}>
                      <img src={i.image} alt={i.name} className="w-28 h-36 object-cover bg-surface-dim hover:opacity-90 transition" />
                    </Link>
                  ) : (
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between gap-4">
                      <div>
                        {i.productId ? (
                          <Link to="/product/$id" params={{ id: i.productId }} className="font-medium hover:opacity-60 transition">{i.name}</Link>
                        ) : (
            ...
            <aside className="border border-hairline p-7 h-fit lg:sticky lg:top-28 space-y-4">
              ...
              {subtotal < threshold ? (
                <div className="pt-2">
                  <p className="text-[11px] text-ink-soft mb-2">Add {currency(threshold - subtotal)} more for free shipping</p>
                  <div className="h-1 bg-surface-dim rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-500" style={{ width: `${Math.min(100, (subtotal / threshold) * 100)}%` }} />
                  </div>
                </div>
              ) : (
                <p className="text-[11px] text-emerald-700 font-medium pt-1">✓ Free shipping on this order</p>
              )}
              <div className="flex justify-between text-sm pt-1"><span className="text-ink-soft">Subtotal</span><span>{currency(subtotal)}</span></div>
