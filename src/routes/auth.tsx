...
export const Route = createFileRoute("/auth")({
  ...
  head: ({ match }) => {
    const mode = (match.search as any)?.mode ?? "signin";
    const title = mode === "signup" ? "Join AESTHETE" : "Sign In | AESTHETE";
    return { meta: [{ title }, { name: "description", content: "Access your AESTHETE account." }] };
  },
  component: AuthPage,
});
          ...
          <form onSubmit={handleEmail} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                {mode === "signin" && (
                  <a href="mailto:hello@aesthete.in?subject=Password Reset" className="text-xs text-ink-soft hover:text-ink underline transition">
                    Forgot password?
                  </a>
                )}
              </div>
              ...
              {mode === "signup" && <p className="text-xs text-ink-soft mt-1.5">Minimum 6 characters.</p>}
            </div>
